...
class FinanceProvider extends ChangeNotifier {
  ...
  Future<void> addSavingsGoal(SavingsGoal g) async {
    _savingsGoals.add(g); await _persist(); notifyListeners();
  }
  Future<void> addToSavings(String id, double amount) async {
    final i = _savingsGoals.indexWhere((s) => s.id == id);
    if (i >= 0) {
      _savingsGoals[i] = _savingsGoals[i].copyWith(currentAmount: _savingsGoals[i].currentAmount + amount);
      await _persist(); notifyListeners();
    }
  }
  ...
  Future<void> _persist() async {
    try {
      ...
      await p.setString('budgets',      jsonEncode(_budgets.map((b) => b.toJson()).toList()));
      if (_selectedAccountId != null) await p.setString('selectedAccountId', _selectedAccountId!);
    } catch (e) { debugPrint('FinanceProvider._persist error: $e'); }
  }

  // ─── Static data ─────────────────────────────────────────────────────────────
  static const List<String> expenseCategories = [
    'Suscripción','Alimentación','Transporte','Entretenimiento','Salud',
    'Educación','Arriendo','Servicios','Ropa','Tecnología','Otros',
  ];
  static const List<String> incomeCategories = [
    'Freelance','Agencia IA','Salario','Inversión','Arriendo','Proyecto','Otros',
  ];
  static const Map<String, String> categoryEmoji = {
    'Suscripción':'📱','Alimentación':'🍽️','Transporte':'🚗','Entretenimiento':'🎬',
    'Salud':'💊','Educación':'📚','Arriendo':'🏠','Servicios':'⚡','Ropa':'👕',
    'Tecnología':'💻','Freelance':'💻','Agencia IA':'🤖','Salario':'💼',
    'Inversión':'📈','Proyecto':'🎯','Otros':'💰',
  };
